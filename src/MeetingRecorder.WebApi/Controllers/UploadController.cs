...
public class UploadController : ApiControllerBase
{
    ...
    /// <summary>Step 3 — check which chunks have arrived and whether the batch is complete.</summary>
    ...
    {
        return Envelope(await _uploadService.GetUploadStatusAsync(batchId, ct));
    }

    /// <summary>
    /// Step 4 — reset a failed/missing chunk so the client can re-upload it
    /// through POST /api/upload/chunk.
    /// </summary>
    [HttpPost("retry")]
    public async Task<ActionResult<ApiResponse<UploadStatusResponse>>> Retry([FromBody] RetryChunkRequest request, CancellationToken ct)
    {
        ...
        return Envelope(await _uploadService.RetryChunkAsync(request.BatchId, request.ChunkNumber, ct),
    ...
    {
        return Envelope(await _uploadService.CompleteUploadAsync(batchId, ct), "Upload completed and recording saved.", StatusCodes.Status201Created);
    }
}
